...
echo "Enter the user name of user you want to add permission "
read username
permission $username
elif [[ "$iput" -eq 6 ]] then
exit
...
echo "$B If You Enabled Script As Command Then Just Type 'boot login'"
read iput
echo $iput
}
...
echo "prepairing directory"
mkdir $mpath
echo "Creating Loop Device "
busybox mknod /dev/block/loop279 b 7 279
echo "Device Creation Compleated"
echo "=>mounting chroot image"
echo "Removing Any Previous Image"
...
echo "$C Enter FileSystem Of Image$G "
read fs
cls
fi
...
$@
fi $G 3$C. Format An Existing Image"
echo "     $G 4$C. Exit"
echo ""
echo ""
echo "Enter Choice  :$G"
